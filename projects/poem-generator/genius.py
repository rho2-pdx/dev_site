...
class Genius(object):
    ...
    def __init__(self, access_token=None):
        ...
        if not self.access_token:
            raise Exception("GENIUS_ACCESS_TOKEN is not set in the environment variables.")
    ...
    def get_lines(self, labels):
        ...
        snippets = []
        for label in labels:
            snippet = self.search_and_extract_lyrics(label) #grab lyrics containing label
            if snippet:
                snippets.append(snippet)
            if len(snippets) >= 3: # only use up to 3, don't really need more, it's arbitrary
                break
